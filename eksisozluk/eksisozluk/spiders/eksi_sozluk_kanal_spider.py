...
from eksisozluk.utils.url_builders import build_static_kanal_url
...
class EksiSpider(scrapy.Spider):
    name = "eksi_sozluk_kanal"
    file_tag = "eksi_sozluk_kanal"
    folder_name_data = "eksi_sozluk_kanal_data"
    folder_name_log = "eksi_sozluk_kanal_log"
    ...
    def start_requests(self):
        ...
        logger.info(f"################################################################################")
        logger.info(f"######### Ekşi Sözlük Kanallar oturum başlama zamanı : {session_timestamp} #########")
        logger.info(f"################################################################################")
        
        
        try:
            urls = build_static_kanal_url()
            if not urls:
                logger.warning("[SPIDER] build_static_kanal_url() boş döndü!")
...
#TODO:!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! only first entry kaldırmayı unutma
